        ...
        <x-slot name="trigger">
           ...
           class="relative">
               <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <span class="absolute bg-v-red text-white rounded-full h-5 w-5 text-xxs flex items-center justify-center -top-1 -right-1 border-2">{{ $notificationCount }}</span>
                @endif
           </button>
        </x-slot>
        <x-slot name="content">
           <div class="space-y-4">
            @if($notifications->isNotEmpty() && !$loader)
                ...
                @click.prevent = "
                    $wire.markAllAsRead();
                "
                class="mb-2 text-sm hover:underline p-2 rounded-md bg-white shadow">
                    Mark all as read
                </button>
               @foreach ($notifications as $notification)
               <button wire:click.prevent="markAsRead('{{ $notification->id }}')" 
                class="flex items-start text-left space-x-4 hover:bg-gray-300 rounded-md p-2">
              ...
              @elseif ($loader)
                ...
                <div class="flex justify-between space-x-4 p-4">
                    <div class="animate-pulse flex items-center space-x-2 flex-1">
