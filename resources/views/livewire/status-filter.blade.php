<nav class="hidden md:flex items-center text-gray-400 justify-between text-xs">
    <ul class="flex uppercase font-semibold space-x-5 pb-3 border-b-4">
       <li><a wire:click.prevent="setStatus('all')" href="#" class="border-b-4 pb-3 @if($status === 'all') border-v-blue text-gray-900 @endif">All Ideas({{ $statusCount['all_statuses'] }})</a></li>
       ...
       <li><a wire:click.prevent="setStatus('in-progress')" href="#"
           class="border-b-4 pb-3 @if($status === 'in-progress') border-v-blue text-gray-900 @endif transition duration-150 ease-in hover:text-gray-900 hover:border-blue">Inprogress({{ $statusCount['in-progress'] }})</a></li>
    </ul>
    <ul class="flex uppercase font-semibold space-x-5 pb-3 border-b-4">
